var redirect_uri = "https://localhost:5001/Spotify"; // change this your value
...
function handleLaatstResponse(){
    if ( this.status == 200 ){
        ...
        data.items.forEach(item =>{
            geluisterdvalue += NumberBuilder(item);
        });
...
function NumberBuilder(item){
    let Albumcard = "<div class='card' style='min-width: 18rem;margin:5px;'>"+ 
    "<img class='card-img-top' src='"+item.track.album.images[0].url+"' alt='Album cover'>"+
        "<div class='card-body'>"+
        "<h2 class='card-title h5'>"+item.track.name+"</h2>"+
        "<p class='card-text'>"+item.track.album.name+"</p>"+
        "</div>"+
    "</div>"
    return Albumcard;
}
...
//Hiermee wordt de huidige user opgevraagd
function GetUser(){
    callApi("GET",USER, null, HandleGetUser)
}
//Dit is wat de api uitmoetvoeren als de connection gelukt is
function HandleGetUser(){
    if ( this.status == 200 ){
        var data = JSON.parse(this.responseText);
        console.log("Deze methode wordt aangeroepen");
        document.getElementById("UserNameBtn").value = "Hallo "+data.display_name;
    }
    //401 betekend dat onze token is verouderd
    else if ( this.status == 401 ){
        refreshAccessToken()
